// Copia de src/utils/time-parser.ts del repo raíz — livekit-agent es un
// deploy Docker aislado (rootDir: src) y no puede importar de fuera de sí
// mismo. Mantener en sync a mano si se toca la lógica de parseo en el original.
...
export function parseSpanishTime(text: string, now: Date = new Date()): Date | null {
  ...
  // "primera hora", "por la mañana/tarde" con no day → tomorrow
